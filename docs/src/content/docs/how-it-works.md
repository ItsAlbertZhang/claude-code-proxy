...
Claude Code sends `x-claude-code-session-id` and sends `x-claude-code-agent-id` for child Agents. The proxy treats a session without an Agent ID as the Main lane and gives every valid child Agent its own lane. Provider affinity, Codex continuation and exact-socket WebSocket reuse, native compaction, and conversational prompt-cache identities are isolated by lane. Route-bound Codex state also rolls over when endpoint, account, credential, or full/lite protocol changes.

Malformed or ambiguous identity headers make the request stateless rather than merging it into Main. Count-token and detected auto-review requests do not publish conversational affinity or state. Agent IDs are converted to domain-separated opaque tokens before they enter provider-facing state; traffic captures redact the raw Agent headers. The Agent headers are observed Claude Code behavior, not a documented stable public API, so client upgrades should retain regression coverage.
