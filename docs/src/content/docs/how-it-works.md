...
Claude Code sends `x-claude-code-session-id` and sends `x-claude-code-agent-id` for child Agents. A valid session header by itself selects the Main lane. A valid session plus a direct Agent header selects that child lane; `x-claude-code-parent-agent-id` is optional lineage and does not select the lane. Provider affinity, Codex continuation and exact-socket WebSocket reuse, native compaction, and conversational prompt-cache identities are isolated by lane. Route-bound Codex state also rolls over when endpoint, account, credential, or full/lite protocol changes.
