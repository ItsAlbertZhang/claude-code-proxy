...
| `CCP_CODEX_PREVIOUS_RESPONSE_ID` | `codex.previousResponseId` | `false` | Enables append-only continuation on the exact originating WebSocket for `1`, `true`, or `yes`; otherwise sends full context. |
| `CCP_CODEX_SERVER_COMPACTION` | `codex.serverCompaction` | `false` | Enables lane- and route-bound native compaction for standard boolean words. |
