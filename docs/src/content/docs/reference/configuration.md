...
Codex auto-review classifier requests use `gpt-5.6-luna` by default. Requests routed through other providers retain their requested model. `CCP_AUTO_REVIEW_MODEL` or `autoReviewModel` selects an explicit registered model for all detected classifier requests. Detection is exact and intentionally narrow: the request must be non-streaming, have no non-empty `tools` array, and contain a system text block beginning `You are a security monitor for autonomous AI coding agents.` The classifier request is stateless even when valid session and Agent headers are present; it cannot consume or publish affinity, sequence, continuation, WebSocket, compaction, Kimi, Cursor, or `Read` state.

`CCP_AUTO_REVIEW_EFFORT` and the top-level `autoReviewEffort` key are disabled by default. A non-empty value other than `off` replaces the request's ordinary `output_config.effort` for every detected classifier request, independently of whether its model changes. The environment setting takes precedence; `off` disables a file-configured effort, while an empty environment value falls through to the file like `CCP_AUTO_REVIEW_MODEL`. The value is not validated at startup: after model routing succeeds, the final provider handles it exactly as if the client had supplied that request effort. Provider routing remains controlled only by the model setting. Normal messages, streaming requests, tool-using requests, and token counting retain their requested model and effort; token counting is also stateless and remains local.
...
| `CCP_CODEX_PREVIOUS_RESPONSE_ID` | `codex.previousResponseId` | `false` | Enables append-only exact-WebSocket continuation for translated Anthropic Messages with `1`, `true`, or `yes`. Native caller-supplied response IDs are independent. |
...
`CCP_CODEX_FULL_LANE` accepts trimmed, case-insensitive `1`, `true`, `yes`, or `on` and `0`, `false`, `no`, or `off`. An empty or unrecognized environment value falls through to `codex.fullLane`, then to the `false` default. The flag affects only translated GPT-5.6 Sol/Terra Messages and their local count-token shape. Luna stays Lite. Native Responses and Chat ignore the flag and use Lite for GPT-5.6 Luna, Sol, and Terra; other registered models use Full. Hosted and standalone search use Full, with hosted Luna upgraded to Sol. Full and Lite are different immutable routes and never share state.

Conversation state uses the strict Claude session/direct-Agent scope. Each feature derives a separate opaque provider lane; raw session, Agent, OpenAI fallback, and public Rust string-adapter values are not built-in state keys or upstream conversation IDs. Missing, duplicate, malformed, or ambiguous Claude identity is stateless. Count-token, detected auto-review, image, transcription, and auxiliary requests also cannot consume or mutate affinity, sequence, continuation, WebSocket, compaction, Kimi, Cursor, or `Read` state.

A Codex request binds one endpoint, known account, credential, and protocol lane before transport starts. Before semantic output, a 401 on route A can construct exactly one route B only for the same known account, changed credentials, and replayable context; unchanged credentials, unknown or changed account, or route B's 401 ends the attempt. After semantic output, the current request is never replayed. Native caller-supplied `previous_response_id` similarly prevents replay after a header 401. A native unauthorized failure found inside an HTTP 200 JSON body or SSE stream preserves the original status, bytes, and framing and refreshes only a later request.

State bounds are fixed rather than configurable. Idle session, continuation, WebSocket, and native-compaction entries expire after 30 minutes; session, continuation, and WebSocket registries cap at 10,000 entries. Continuation transcripts cap at 2 MB per owner and 20 MB total. Native compaction caps at 1,000 entries, 4 MiB each, and 20,000,000 bytes total. Codex `Read` correction metadata caps at 4,096 notes and remains on the stable Main/direct-Agent lane across route rollover while sibling Agents stay isolated.
