...
- Codex state is also bound to endpoint, account, credential generation, and full/lite protocol. Route rollover starts a fresh state lane instead of reusing incompatible state. Proxy-local `Read` correction metadata remains downstream-lane scoped so it can explain a tool result after that rollover.
- Before downstream semantic output, a 401 can rebuild the complete bound attempt once only when refreshed and rejected auth have the same known account, produce a fresh route binding, and the proxy has complete replayable context. A caller-supplied native `previous_response_id` is not replayed because its upstream state cannot be reconstructed. A changed or unknown account, unchanged credential, or route B's 401 returns the current request's 401 without another replay; refreshed auth remains available to a later request. After semantic output, no replay occurs. The Anthropic adapter preserves published output and emits a sanitized Anthropic stream error rather than leaking the raw Codex 401 frame, while native in-band unauthorized failures can refresh only the next route.
