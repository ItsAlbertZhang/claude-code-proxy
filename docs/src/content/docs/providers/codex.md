...
This option affects only Anthropic Messages translation and its token-count shape. Native `/v1/responses` requests retain their existing lane behavior. The full-lane contract is specific to the default ChatGPT backend; custom `CCP_CODEX_BASE_URL` endpoints may not support it.
...
WebSocket setup honors `HTTP_PROXY` for `ws://`, `HTTPS_PROXY` for the default `wss://` endpoint, `ALL_PROXY` as a fallback, and `NO_PROXY` exclusions. A normal HTTP proxy can therefore carry the default WebSocket connection with CONNECT; TUN mode is not required. Set proxy variables before starting the process and restart after changing them. For example, `HTTPS_PROXY=http://127.0.0.1:7890` means “send HTTPS/WSS destinations through the HTTP proxy at port 7890.” It does not require an `https://` proxy URL.
